...
| **Code 39 Extended** | 读（解码器选项，无格式常量） | 读（有独立常量） | **读 + 独立常量** |
| **Code 32（意大利药品码）** | — | 读（写：zint） | **已支持** |
| **PZN（德国药品码）** | — | 读（写：zint） | **已支持** |
| ITF-14 | 读（报为 ITF） | 读（报为 ITF） | 读（报为 ITF） |
| EAN-13、EAN-8、UPC-A、UPC-E | 读 + 写 | 读 + 写（原生） | 读 + 写 |
| EAN-2 / EAN-5 | 读（`UPC_EAN_EXTENSION`） | 不可单独解码 | 读 |
| ISBN | 读（报为 EAN-13） | 读（报为 EAN-13） | 读（报为 EAN-13） |
...
**ITF-14 和 ISBN 是同样的情况，也是同样的方式发现的**：两者都只出现在 zxing-cpp 的
格式枚举里，任何 reader 都不会返回它们。它的 ITF reader 报 `ITF`，EAN/UPC reader 报
`EAN13`——和 Java 侧完全一样。

所以最初对比表里的九项，**有三项根本不是缺口**。这些更正是明写出来而不是悄悄改掉的，
因为"**枚举差异看起来像能力差异**"正是本项目一开始就想避开的陷阱——而它把本项目坑了
三次。
